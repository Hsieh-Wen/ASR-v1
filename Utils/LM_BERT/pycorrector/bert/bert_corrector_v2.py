...
class BertCorrector(Corrector):
    def __init__(self, bert_model_dir, topk=5):
...
#            logger.debug('Loaded bert model: %s, spend: %.3f s.' % (bert_model_dir, time.time() - t1))
        ...
        self.ws, self.dictionary = self.init_chkip(path_of_dict_txt="special_word.txt")
...
#        print(bert_model_dir)
    ...
    def bert_correct(self, text, use_confusion_word=True):
        """
        句子纠错
        :param text: 句子文本
...
#        model_lm.to(device)
    ...
    def lm_predict(self, sentence, device="cuda"):
        """
        ...
        if device == "cuda":
            ...
            token = self.tokenizer_lm(sentence)
...
#        print(d)
    ...
    def init_chkip(self, path_of_dict_txt, ws_path="./Ckip_Tagger/data/"):
        """
    ...
    def bert_correct_wwm(self, text, wwm, reverse=False, use_confusion_word=False, topk=5):
        """
        句子纠错
        :param text: 句子文本
        :return: corrected_text, list[list], [error_word, correct_word, begin_pos, end_pos]
...
#                print(word_sentence_list)
            ...
            blk_new, details = self.predict_fc(word_sentence_list, details, start_idx)
    ...
    def predict_fc(self, word_sentence_list, details, start_idx):
        ...
        if self.reverse:
            ...
            for idx, s in zip(idxs[::-1], word_sentence_list[::-1]):
                ...
                s, details = self.predict_sentence_mask(sentence_lst, s, details, start_idx, idx)
    ...
    def predict_sentence_mask(self, sentence_lst, s, details, start_idx, idx):
        """
        ...
        if is_chinese_string(s):
            ...
            s, details = self.predict_word(sentence_new, s, details, start_idx, idx)
    ...
    def predict_word(self, sentence_new, s, details, start_idx, idx):
