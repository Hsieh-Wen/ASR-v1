...
class CheckpointCallback(NeMoCallback):
    ...
    # def __save_to(self, path, state): #1091029
    ...
    def on_step_end(self, state):
        # step = state["step"]
        # print("on_step_end_epoch_freq: ",self._epoch_freq)# 1091029
        # print("Eval_step: ",str(step))# 1091029

        # if self._step_freq > 0 and step % self._step_freq == 0 and step > 0:# 1091029
        if self._step_freq > 0 :      # 1091029      
            self.__save_to(self._folder, state)
            self._step_freq = -1# 1091029
            
    def on_action_end(self, state):
        # print("on_action_end_epoch_freq: ",self._epoch_freq)# 1091029
        # if self._step_freq > 0 or self._epoch_freq > 0:# 1091029
        if self._step_freq > 0 :# 1091029
            self.__save_to(self._folder, state)
            self._step_freq = -1# 1091029
            
    def on_epoch_end(self, state):
        # print("on_epoch_end_epoch_freq: ",self._epoch_freq)
        epoch = state["epoch"]
        # if self._epoch_freq > 0 and epoch % self._epoch_freq == 0 and epoch > 0: # 1091029
        if self._epoch_freq != -1 :# 1091029
            self.__save_to(self._folder, state)
            self._epoch_freq = -1# 1091029
