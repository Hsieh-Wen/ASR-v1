...
class EvaluatorCallback(ActionCallback):
    ...
    def __init__(
        ...
        # Save model parmeters (1091104)
        # self.tag = tag
        # self.eval_metric = eval_metric
        # self.wer_list = wer_list
    ...
    def on_epoch_end(self):#,state=1
        pass #1091104
     
        # if self.tag is None:
        #     dict_name = "Evaluation_" + self.eval_metric
        #     # wer_dict = self.user_done_callback[f'Evaluation_{self.eval_metric}']
        # else:
        #     dict_name = "Evaluation_" + self.eval_metric + "_" + self.tag
        #     # tag=self.eval_metric
        #     # eval_metric = self.eval_metric
        #     # wer_dict = self.user_done_callback[f"Evaluation_{eval_metric}_{tag}"]
        # print("dict_name: ", dict_name)
        
        # print("user_done_callback: ",self.user_done_callback)
        # print(type(self.user_done_callback))
        
        # with open('var_data.json', 'r') as f:
        #     var_data = json.load(f)
        # wer_dict = self.user_done_callback(var_data)
        
        # print("wer_dict: ",wer_dict)
        # wer = wer_dict['{dict_name}']
        # print("wer: ", wer)
        # self.wer_list.append(wer)
        # print("WERs: ",self.wer_list)
        # if  self.wer_list != []:
        #     min_index=np.argmin(self.wer_list)            
        #     if  len(self.wer_list) ==1 or (len(self.wer_list)-1 == min_index and self.wer_list[min_index] <= self.save_wer_threshold) :   # 存取第1個 model 以及 低於save_wer_threshold的Model
        #         print("Save Model !!!")
        #         # self.__save_to(self.save_folder_path, state)
