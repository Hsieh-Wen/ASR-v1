...
#                    filename = f"{module}-STEP-every5.pt"

                else:
                    filename = f"{module}-EPOCH-{state['epoch']}.pt"
#                    filename = f"{module}-EPOCH-every5.pt"
...
#            filename = f"trainer-EPOCH-every5.pt"
    ...
    def on_epoch_end(self, state):
        epoch = state["epoch"]
        print(f"epoch={epoch}")
        print(f"_epoch_freq={self._epoch_freq}")
        print(f"epoch % self._epoch_freq= {epoch % self._epoch_freq}")
        if self._epoch_freq > 0 and int(epoch % self._epoch_freq) == 0 and epoch > 0:
            self.__save_to(self._folder, state)



class SaveModelinWERCallback(NeMoCallback):
    """A callback that does checkpointing of module weights and trainer (incl. optimizer) status.
    ...
    def __init__(
        ...
        self._folder = folder
        # self._epoch_freq = epoch_freq
        # self.wer_list = wer_epoch_result
        self.save_wer_threshold = save_wer_threshold
        self.gpu_num  = gpu_num 
        self.wer_data_file = wer_data_file
    ...
    def __save_to(self, path, state):
        ...
        for module in AppState().modules:
            if module.num_weights > 0:
                ...
                filename = f"{module}-EPOCH-{save_model_name}.pt"
                module.save_to(os.path.join(path, filename))

        filename = f"trainer-EPOCH-{save_model_name}.pt"
        state.save_state_to(f"{path}/{filename}")
        # self._saved_ckpts.append(f"-{state['epoch']}.pt")
    ...
    def on_epoch_end(self, state):
        ...
        if os.path.isfile(self.wer_data_file ):
            fp = open(self.wer_data_file, "r")
            WERs = fp.readlines()
            fp.close()
            ...
            for item in WERs:
                string=item.split("\n",1)
                wer_list.append(float(string[0]))        
            print("WER_List: ",wer_list)
    
    
            # if len(wer_list) == 2:
            #     wer_list[1] = wer_list[0]-0.2
            
            # if len(wer_list) == 3:
            #     wer_list[2] = wer_list[1]-0.55
        
            # if len(wer_list) == 5:
            #     wer_list[4] = wer_list[2]-0.8
                
            # print ("Test_WERS: ", wer_list)  
            
            if  wer_list != []:
                min_index=np.argmin(wer_list)            
                # and (epoch+1)% self.gpu_num == 0
                if  epoch ==0 or (len(wer_list)-1 == min_index and wer_list[min_index] <= self.save_wer_threshold ) :   # 存取第2個 model 以及 低於save_wer_threshold的Model
                    print("Save Model !!!")
                    self._epoch_freq = 1
                    self.__save_to(self._folder, state)
