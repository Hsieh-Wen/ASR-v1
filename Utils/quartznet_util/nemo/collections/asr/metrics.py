...
def WER_result_list(hypotheses: List[str], references: List[str], use_cer=False, wer_list= List[float],acc_list= List[float]) -> float:
    """
    Computes Average Word Error rate between two texts represented as
    corresponding lists of string. Hypotheses and references must have same
    length.
    ...
    for h, r in zip(hypotheses, references):
        ...
        one_words = len(r_list)
        one_scores = __levenshtein(h_list, r_list)
        one_wer =  1.0 * one_scores / one_words
        wer_list.append(one_wer)
        acc_list.append(1-one_wer)
