...
from .metrics import classification_accuracy, word_error_rate, WER_result_list
...
def process_evaluation_epoch(global_vars: dict, wer_data_file: str,  eval_metric='WER', tag=None):
    """
    Calculates the aggregated loss and WER across the entire evaluation dataset
    """


    # print("Vars:",global_vars)
    ...
    f2 = open(wer_data_file , 'a+', encoding='utf-8')
    f2.write(str(save_wer)+"\n")
    f2.close()  
    print("Save txt !!")
    if tag is None:
