...
def process_evaluation_epoch_and_save_model(global_vars: dict,all_wer: list,folder: str, call_save: list, save_wer_threshold: float, eval_metric='WER', tag=None): # 1091029
    """
    Calculates the aggregated loss and WER across the entire evaluation dataset
    """
    eloss = torch.mean(torch.stack(global_vars['EvalLoss'])).item()
    ...
    # if len(all_wer) == 3:
    #     all_wer[2] = all_wer[1]-0.1

    # if len(all_wer) == 5:
    #     all_wer[4] = all_wer[2]-0.5
        
    #print ("WERS: ", all_wer)
    print("folder: ", folder)
    
    if folder != None and all_wer != []:
        if not os.path.isdir(folder): # 如果未創建資料夾
            print("Bulid folder!!")
            os.mkdir(folder) 
   
        print("Run process_evaluation_epoch_and_save_model !!!!!!!!!!!!!!!!!!!!!")
        
        min_index=np.argmin(all_wer)
        save_point = -1
        checkpointer_callback = 0
        print("min_index : ",min_index)
        print("min_wer: ",all_wer[min_index] )
            # save best model
        if  len(all_wer)==2 or (len(all_wer)-1 == min_index and all_wer[min_index] <= save_wer_threshold) :   # 存取第2個 model 以及 低於save_wer_threshold的Model
            print("Save Model !!!")
            save_point = 1
            print("save_point: ",save_point)
            checkpointer_callback = nemo.core.CheckpointCallback(folder=folder,epoch_freq=save_point,step_freq=-1)
            # call_save.append(checkpointer_callback)
        else:
            print("NO Save Model !!!")
            checkpointer_callback = nemo.core.CheckpointCallback(folder=folder,step_freq=-1,epoch_freq=-1)
        call_save.append(checkpointer_callback)
