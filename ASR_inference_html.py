...
class HtmlResult():
    ...
    def load_models_data_to_dict(self):
        ...
        for i in range(1,len(self.compare_data_path_list)):
            ...
            # 將資料存入字典
            model_data_dict[self.column_name_list[i]] = {'label_list': list(label_list), 'predict_list': list(predict_list), 'wer_list': n_wer_list,'wer_avg': wer_avg }  
    ...
    def dict_to_html(save_path, ground_truth, compare_model_path_dict, data_dict):      
        ...
        * Input:
        ...
        for i in range(len(compare_model_path_dict)):
            ...
            f_html.write('<td>' + input_csv_name[i] + "= " + compare_model_path_dict[input_csv_name[i]]+ "\n" + '</td>')
            f_html.write('</font>')
        f_html.write('</br>')
        f_html.write('</tr>')
